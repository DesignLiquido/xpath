...
function parseXsd(xsd: string): SchemaRegistry {
  ...
  const elementRegex = /<\w*:?element[^>]*name\s*=\s*"([^"]+)"[^>]*type\s*=\s*"([^"]+)"[^>]*\/>/gi;
  const attributeRegex = /<\w*:?attribute[^>]*name\s*=\s*"([^"]+)"[^>]*type\s*=\s*"([^"]+)"[^>]*\/>/gi;
...
export function applySchemaToStaticContext(context: XPathStaticContext, registry: SchemaRegistry): void {
