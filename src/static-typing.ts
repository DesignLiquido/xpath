...
export function concatCardinality(a: SequenceCardinality, b: SequenceCardinality): SequenceCardinality {
...
export function inferSequence(items: StaticType[]): StaticType {
  ...
  const card = items.some(i => i.cardinality !== 'exactly-one') ? 'zero-or-more' : 'one-or-more';
...
export function inferType(node: any, env: TypeEnv): StaticType {
  ...
  switch (node.kind) {
    ...
    case 'BinaryExpression': {
      ...
      if (['eq', 'ne', 'lt', 'le', 'gt', 'ge', '=', '!=', '<', '<=', '>', '>='].includes(node.operator)) {
