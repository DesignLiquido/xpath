...
export const ERROR_CODES: Record<string, ErrorCodeMetadata> = {
  ...
  XPTY0020: {
    ...
    description:
      'A path expression was evaluated with a non-node context item.',
  ...
  FOTY0012: {
    ...
    description:
      'String value of an element with element-only content cannot be extracted.',
...
export function getErrorCodesByType(type: 'static' | 'dynamic' | 'type'): string[] {
  return Object.keys(ERROR_CODES).filter(
    code => ERROR_CODES[code].type === type,
  );
}
