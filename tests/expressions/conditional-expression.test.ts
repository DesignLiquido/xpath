...
describe('XPath 2.0 Conditional Expression (Phase 3.1)', () => {
    ...
    describe('EBV in test condition', () => {
        it('true boolean selects then branch', () => {
            const expr = new XPathConditionalExpression(
                new LiteralExpression(true),
                new LiteralExpression('T'),
                new LiteralExpression('F')
            );
        ...
        it('single-item sequence uses EBV of item', () => {
            const expr1 = new XPathConditionalExpression(
                new LiteralExpression([0]),
                new LiteralExpression('T'),
                new LiteralExpression('F')
            );
            expect(expr1.evaluate(ctx)).toBe('F');
            const expr2 = new XPathConditionalExpression(
                new LiteralExpression(['']),
                new LiteralExpression('T'),
                new LiteralExpression('F')
            );
            expect(expr2.evaluate(ctx)).toBe('F');
            const expr3 = new XPathConditionalExpression(
    ...
    describe('Short-circuit branch evaluation', () => {
        ...
        it('evaluates only the selected branch', () => {
            ...
            const exprTrue = new XPathConditionalExpression(
                new LiteralExpression(true),
                thenExpr,
                elseExpr
            );
            ...
            const exprFalse = new XPathConditionalExpression(
                new LiteralExpression(false),
                thenExpr2,
                elseExpr2
            );
