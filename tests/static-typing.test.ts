import { XS, exactlyOne, inferLiteral, inferArithmetic, inferComparison, inferSequence, inferType } from '../src/static-typing';
...
describe('Static Typing (minimal scaffolding)', () => {
  ...
  test('inferType basic AST nodes', () => {
    ...
    expect(inferType({ kind: 'StringLiteral', value: 'x' }, env)).toEqual(exactlyOne(XS.string));
    expect(inferType({ kind: 'NumericLiteral', value: '5' }, env)).toEqual(exactlyOne(XS.double));
    expect(inferType({ kind: 'BooleanLiteral', value: true }, env)).toEqual(exactlyOne(XS.boolean));

    const bin = { kind: 'BinaryExpression', operator: '+', left: { kind: 'NumericLiteral', value: '1' }, right: { kind: 'NumericLiteral', value: '2' } };
    expect(inferType(bin, env)).toEqual(exactlyOne(XS.double));

    const cmp = { kind: 'BinaryExpression', operator: 'eq', left: { kind: 'StringLiteral', value: 'a' }, right: { kind: 'StringLiteral', value: 'b' } };
    expect(inferType(cmp, env)).toEqual(exactlyOne(XS.boolean));

    const seq = { kind: 'SequenceExpression', items: [{ kind: 'StringLiteral', value: 'a' }, { kind: 'NumericLiteral', value: '2' }] };
